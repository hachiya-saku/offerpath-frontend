    ...
    <div className="grid min-h-screen w-full max-w-screen grid-cols-[252px_minmax(0,1fr)] bg-[#0c0b0e] max-[1050px]:grid-cols-1">
      <aside className={`fixed inset-y-0 left-0 z-30 flex w-[252px] flex-col border-r border-[#211e25] bg-[#111014] px-4 pb-[18px] pt-6 transition-transform duration-200 max-[1050px]:-translate-x-full ${menuOpen ? 'max-[1050px]:translate-x-0' : ''}`}>
        <div className="flex items-center gap-3 px-2 pb-[30px]">
          ...
          <Button className="ml-auto hidden border-[#2c2831] text-[#a39ca9] max-[1050px]:inline-flex" variant="ghost" size="icon" type="button" onClick={() => setMenuOpen(false)} aria-label="关闭导航"><X size={18} /></Button>
        </div>
        ...
        <div className="mt-auto flex items-center gap-2 border-t border-[#211e25] px-1.5 pt-3.5">
          ...
          <Button className="border-[#2c2831] text-[#a39ca9] hover:bg-[#1b181f] hover:text-white" variant="ghost" size="icon" type="button" title="退出登录" onClick={() => navigate('/login')}><LogOut size={17} /></Button>
        </div>
      </aside>
      {menuOpen && <button className="fixed inset-0 z-20 hidden border-0 bg-black/60 max-[1050px]:block" aria-label="关闭导航" onClick={() => setMenuOpen(false)} />}
      <div className="col-start-2 min-w-0 overflow-x-hidden max-[1050px]:col-start-1">
        <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-[#211e25] bg-[#0c0b0e]/90 px-8 backdrop-blur-2xl max-[760px]:h-16 max-[760px]:px-4">
          <div className="flex items-center gap-3">
            <Button className="hidden border-[#2c2831] text-[#a39ca9] max-[1050px]:inline-flex" variant="ghost" size="icon" type="button" onClick={() => setMenuOpen(true)} aria-label="打开导航"><Menu size={20} /></Button>
          ...
          <div className="flex items-center gap-2.5 max-[460px]:absolute max-[460px]:right-4">
            <label className="flex h-[38px] w-[245px] items-center gap-2 rounded-[5px] border border-[#2c2831] bg-[#111014] px-2.5 text-[#6f6977] max-[760px]:hidden"><Search size={16} /><input className="min-w-0 flex-1 border-0 bg-transparent text-xs text-[#f5f2f7] outline-none" aria-label="快速搜索" placeholder="搜索岗位..." /><kbd className="rounded border border-[#2c2831] px-1.5 py-0.5 text-[9px] text-[#77717f]">⌘ K</kbd></label>
            <Button className="h-[38px] rounded-[5px] bg-[#7c3aed] px-3.5 text-xs text-white hover:bg-[#8b4cf0] max-[460px]:size-[37px] max-[460px]:px-0 max-[460px]:text-[0]" type="button" onClick={() => navigate('/jobs/new')}><Plus size={17} />新增岗位</Button>
          </div>
        </header>
