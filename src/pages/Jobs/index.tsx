...
export function Jobs() {
  ...
  const jobs = useMemo(
    () =>
      apiJobs.map((job) => ({
        ...
        updatedAtValue: Date.parse(job.updatedAt),
  ...
  if (isLoading) {
    return (
      <div className="grid animate-pulse gap-4">
        <div className="h-24 rounded-md bg-[#151318]" />
        <div className="h-72 rounded-md bg-[#151318]" />
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <section className="grid min-h-72 place-items-center rounded-md border border-[#30252d] bg-[#151318]">
        <p className="text-xs text-[#b8b1bd]">
          {language === "ja"
            ? "求人データを読み込めませんでした。"
            : "岗位数据加载失败。"}
        </p>
      ...
      >
            ...
            >
              ...
              {statuses.map((item) => (
                <option key={item} value={item}>
                  {getJobStatusLabel(item, language)}
                </option>
              ))}
            </select>
          </label>
        ...
        <div className="grid grid-cols-[minmax(290px,1.2fr)_repeat(4,minmax(140px,.65fr))_auto] items-center gap-2 p-3 max-[760px]:grid-cols-2 max-[460px]:grid-cols-1">
          ...
          <label className={advancedLabelClass}>
            <span className="shrink-0 text-[#6f6977]">
              {text.requiredSkill}
            </span>
            <select
...
const jobsCopy = {
  ja: {
    title: "求人一覧",
    subtitle: "求人情報を絞り込み、比較しながら継続的に管理できます。",
    results: "件",
    filters: "求人フィルター",
    search: "企業名・職種名を検索",
    ...
    newest: "更新が新しい順",
    oldest: "更新が古い順",
    highestMatch: "マッチ度が高い順",
    highestSalary: "給与が高い順",
    annualSalary: "年収",
    minimum: "最低",
    to: "〜",
    maximum: "最高",
    requiredSkill: "必須スキル",
    allRequiredSkills: "すべての必須スキル",
    bonusSkill: "歓迎スキル",
    allBonusSkills: "すべての歓迎スキル",
    location: "勤務地",
    ...
    emptyHint: "キーワードや絞り込み条件を変更してください。",
    clearFilters: "フィルターをクリア",
    pagination: "1 / 1 ページ",
  },
  zh: {
    title: "所有岗位",
    subtitle: "筛选、比较并持续维护你的求职机会。",
    results: "条结果",
    filters: "岗位筛选",
    search: "搜索公司或岗位",
    allStatuses: "全部状态",
    ...
    maximum: "最高",
    requiredSkill: "必须技能",
    allRequiredSkills: "全部必须技能",
    bonusSkill: "加分技能",
    allBonusSkills: "全部加分技能",
    location: "工作地点",
    allLocations: "全部地点",
    minimumMatch: "最低匹配度",
    unlimited: "不限",
    ...
    updatedAt: "更新时间",
    empty: "没有符合条件的岗位",
    emptyHint: "尝试调整关键词或筛选条件。",
    clearFilters: "清除全部筛选",
    pagination: "第 1 页，共 1 页",
  },
} as const;
