...
const textareaClass =
  "w-full resize-y rounded-[5px] border border-[#2c2831] bg-[#100f12] p-[11px] text-xs leading-6 text-[#f5f2f7] outline-none transition-colors focus:border-[#674a98]";

const copy = {
  ja: {
    back: "戻る",
    title: "求人を登録",
    subtitle: "応募判断と進捗管理に必要な情報をまとめます。",
    cancel: "キャンセル",
    save: "求人を保存",
    basic: "基本情報",
    basicHint: "求人と企業を識別するための基本項目",
    companyName: "企業名",
    companyPlaceholder: "例：株式会社サンプル",
    positionName: "職種名",
    positionPlaceholder: "例：フロントエンドエンジニア",
    employmentType: "雇用形態",
    hiringCount: "採用予定人数",
    hiringCountSuffix: "名",
    platform: "求人媒体",
    workMode: "勤務形態",
    location: "勤務地",
    locationPlaceholder: "例：東京都 / フルリモート",
    salary: "給与条件",
    salaryHint: "年収・月給・時給と固定残業代を分けて記録",
    annualSalary: "年収",
    monthlySalary: "月給",
    hourlySalary: "時給",
    salaryMin: "下限",
    salaryMax: "上限",
    currency: "通貨",
    fixedOvertime: "固定残業代を含む",
    overtimeHours: "固定残業時間",
    overtimeHoursSuffix: "時間 / 月",
    overtimeAmount: "固定残業代",
    overtimeAmountSuffix: "円 / 月",
    sourceUrl: "求人URL",
    descriptions: "募集要項",
    descriptionsHint: "求人票の内容を項目ごとに整理",
    jobDescription: "仕事内容",
    jobDescriptionPlaceholder: "担当業務、プロダクト、役割、期待される成果など",
    ...
    skills: "スキル要件",
    skillsHint: "マッチ度計算に使用する技術要件",
    requiredSkills: "必須スキル",
    bonusSkills: "歓迎スキル",
    tagPlaceholder: "入力後 Enter",
    notes: "個人メモ",
    notesHint: "応募準備や面接で確認したい内容",
    notesPlaceholder: "確認事項、面接準備、担当者情報など",
    tracking: "応募状況",
  ...
  zh: {
    back: "返回",
    title: "记录新岗位",
    subtitle: "集中记录判断是否投递和跟踪进度所需的信息。",
    cancel: "取消",
    save: "保存岗位",
    basic: "基本信息",
    basicHint: "用于识别岗位和公司的基本字段",
    companyName: "公司名称",
    companyPlaceholder: "例如：株式会社サンプル",
    positionName: "岗位名称",
    ...
    workMode: "工作方式",
    location: "工作地点",
    locationPlaceholder: "例如：东京 / 全远程",
    salary: "薪资条件",
    salaryHint: "分别记录年薪、月薪、时薪和固定加班费",
    annualSalary: "年薪",
    monthlySalary: "月薪",
    hourlySalary: "时薪",
    salaryMin: "最低",
    salaryMax: "最高",
    currency: "货币",
    fixedOvertime: "包含固定加班费",
    overtimeHours: "包含加班时长",
    overtimeHoursSuffix: "小时 / 月",
    overtimeAmount: "固定加班金额",
    overtimeAmountSuffix: "日元 / 月",
    sourceUrl: "岗位链接",
    descriptions: "岗位描述",
    descriptionsHint: "将招聘信息按实际用途拆分记录",
    jobDescription: "工作内容",
    jobDescriptionPlaceholder: "负责的业务、产品、角色和预期成果等",
    ...
    skills: "技能要求",
    skillsHint: "用于计算匹配度的技术要求",
    requiredSkills: "必须技能",
    bonusSkills: "加分技能",
    tagPlaceholder: "输入后按 Enter",
    notes: "个人备注",
    notesHint: "记录应聘准备和面试中需要确认的内容",
    notesPlaceholder: "确认事项、面试准备、联系人信息等",
    tracking: "投递状态",
    currentStatus: "当前状态",
...
export function JobForm() {
  ...
  const submit = async (event: SubmitEvent<HTMLFormElement>) => {
    ...
    const data: CreateJobRequest = {
      ...
      positionName: getRequiredString(formData, "positionName"),
      platform: getRequiredString(formData, "platform"),
      employmentType: getRequiredString(
        formData,
        "employmentType",
      ) as EmploymentType,
      hiringCount: getOptionalNumber(formData, "hiringCount"),
      workMode: getRequiredString(formData, "workMode") as WorkMode,
      location: getOptionalString(formData, "location"),

      annualSalaryMin: getOptionalNumber(formData, "annualSalaryMin"),
      annualSalaryMax: getOptionalNumber(formData, "annualSalaryMax"),
      monthlySalaryMin: getOptionalNumber(formData, "monthlySalaryMin"),
      monthlySalaryMax: getOptionalNumber(formData, "monthlySalaryMax"),
      hourlySalaryMin: getOptionalNumber(formData, "hourlySalaryMin"),
      hourlySalaryMax: getOptionalNumber(formData, "hourlySalaryMax"),
      salaryCurrency: getOptionalString(formData, "salaryCurrency"),

      includesFixedOvertime,
      fixedOvertimeHours: includesFixedOvertime
        ? getOptionalNumber(formData, "fixedOvertimeHours")
        : undefined,
      fixedOvertimeAmount: includesFixedOvertime
      ...
      preferredQualifications: getOptionalString(
        formData,
        "preferredQualifications",
      ),
      selectionProcess: getOptionalString(formData, "selectionProcess"),
      workLocationDetails: getOptionalString(formData, "workLocationDetails"),
      workingHours: getOptionalString(formData, "workingHours"),
      benefits: getOptionalString(formData, "benefits"),
      holidays: getOptionalString(formData, "holidays"),
      teamEnvironment: getOptionalString(formData, "teamEnvironment"),
      ...
      <section className="flex items-end justify-between gap-7 pb-1 max-[760px]:grid max-[760px]:items-start">
        ...
        <div className="flex gap-2 max-[760px]:w-full max-[760px]:[&>button]:flex-1">
          ...
          {saveError && (
            <p
              role="alert"
              className="rounded-[5px] border border-[#5a2e36] bg-[#27171c] px-4 py-3 text-xs text-[#ef9aa2]"
            >
              {saveError}
            </p>
          )}
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_300px] items-start gap-[18px] max-[1050px]:grid-cols-1">
        <div className="grid gap-[18px]">
          ...
          >
            ...
            <div className="mt-4 grid gap-3">
              ...
              <SalaryRange
                ...
                minLabel={text.salaryMin}
                maxLabel={text.salaryMax}
                minName="monthlySalaryMin"
                maxName="monthlySalaryMax"
                minPlaceholder="30"
                maxPlaceholder="45"
                suffix={language === "ja" ? "万円" : "万日元"}
              />
              <SalaryRange
                ...
                minLabel={text.salaryMin}
                maxLabel={text.salaryMax}
                minName="hourlySalaryMin"
                maxName="hourlySalaryMax"
                minPlaceholder="1500"
                maxPlaceholder="2500"
                suffix={language === "ja" ? "円 / 時" : "日元 / 时"}
              />
            </div>
            <label className="mt-5 flex w-fit items-center gap-2.5 text-[11px] text-[#c8c2cd]">
              <input
                className="size-4 accent-[#8b5cf6]"
                type="checkbox"
                name="includesFixedOvertime"
                checked={includesFixedOvertime}
                onChange={(event) =>
                  setIncludesFixedOvertime(event.target.checked)
                }
              />
              {text.fixedOvertime}
            </label>
              ...
              <div className="mt-4 grid grid-cols-2 gap-[18px] max-[560px]:grid-cols-1">
                <Field label={text.overtimeHours}>
                  <InputWithSuffix
                    ...
                    min="0"
                    placeholder="20"
                    suffix={text.overtimeHoursSuffix}
                  />
                </Field>
                <Field label={text.overtimeAmount}>
                  <InputWithSuffix
                    ...
                    min="0"
                    placeholder="50000"
                    suffix={text.overtimeAmountSuffix}
                  />
                </Field>
              </div>
            )}
          ...
          <FormSection
            ...
            title={text.skills}
            hint={text.skillsHint}
          >
            <TagEditor
              label={text.requiredSkills}
              placeholder={text.tagPlaceholder}
              tags={required}
              setTags={setRequired}
            />
            <TagEditor
              label={text.bonusSkills}
              placeholder={text.tagPlaceholder}
              tags={bonus}
              setTags={setBonus}
            />
        ...
        <aside>
          <div className="sticky top-[100px] rounded-md border border-[#211e25] bg-[#151318] p-[21px] max-[1050px]:static">
            ...
            <div className="mt-[18px] rounded-[5px] border border-[#2d2840] bg-[#1b1725] p-3.5">
              ...
              <strong className="mt-1 block text-[11px] text-[#b69bf2]">
                {text.savedDatesValue}
              </strong>
            </div>
    ...
    <section className={sectionClass}>
      <div className={sectionTitleClass}>
        ...
        <div>
          ...
          <p className="mt-1 text-[10px] text-[#948e9d]">{hint}</p>
        </div>
      </div>
      {children}
    </section>
...
function InputWithSuffix({
  ...
  return (
    <div className="relative flex items-center">
      <input className={`${fieldClass} pr-[88px]`} {...props} />
      <span className="pointer-events-none absolute right-[11px] text-[9px] text-[#948e9d]">
        {suffix}
      </span>
    </div>
  );
