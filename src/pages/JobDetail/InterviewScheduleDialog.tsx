...
type Props = {
  job: Job;
  availableStatuses: CreateInterviewRequest["round"][];
  isSaving: boolean;
  error: string;
  onClose: () => void;
  onSave: (data: CreateInterviewRequest) => Promise<void>;
};
...
export function InterviewScheduleDialog({
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    void onSave({
      ...
      platform: mode === "ONLINE" ? value("platform") : undefined,
      meetingUrl: mode === "ONLINE" ? value("meetingUrl") : undefined,
      meetingId: mode === "ONLINE" ? value("meetingId") : undefined,
      meetingPassword:
        mode === "ONLINE" ? value("meetingPassword") : undefined,
      location: mode === "OFFLINE" ? value("location") : undefined,
      notes: value("notes"),
    });
  };
      ...
      <section className="my-auto w-full max-w-[620px] overflow-hidden rounded-md border border-[#393140] bg-[#151318] shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="interview-dialog-title">
        <header className="flex items-start justify-between border-b border-[#28232d] px-6 py-5">
          <div>
            ...
            <p className="mb-0 mt-1 text-[11px] text-[#8f8998]">{job.company.name} · {getJobStatusLabel(selectedStatus, language)}</p>
          </div>
        ...
        <form className="grid gap-5 p-6" onSubmit={handleSubmit}>
          <label className="grid gap-2 text-[10px] text-[#8f8998]">
            ...
            <select
              ...
              onChange={(event) =>
                setSelectedStatus(
                  event.target.value as CreateInterviewRequest["round"],
                )
              }
            >
          ...
          {error && <p className="m-0 text-[10px] text-[#ef9aa2]" role="alert">{error}</p>}
          <footer className="flex justify-end gap-2 border-t border-[#28232d] pt-5">
            <Button disabled={isSaving} type="button" variant="outline" className="border-[#35303a] bg-transparent text-[#aaa4ae] hover:bg-[#211d25]" onClick={onClose}>{text.cancel}</Button>
            <Button disabled={isSaving} type="submit" className="bg-[#7c3aed] text-white hover:bg-[#8b4cf0]"><CalendarClock size={16} />{isSaving ? text.saving : text.save}</Button>
          </footer>
        </form>
      </section>
    </div>
...
const dialogCopy = {
  ja: { title: "次の面接を設定", close: "閉じる", round: "面接段階", mode: "面接形式", online: "オンライン", offline: "対面", dateTime: "日時 *", platform: "利用ツール *", meetingUrl: "参加リンク", meetingId: "ミーティング ID", password: "パスコード", location: "会場・住所 *", locationPlaceholder: "東京都千代田区...", notes: "メモ", cancel: "キャンセル", save: "保存してステータスを進める", saving: "保存中..." },
  zh: { title: "安排下一轮面试", close: "关闭", round: "面试阶段", mode: "面试形式", online: "线上", offline: "线下", dateTime: "日期与时间 *", platform: "会议平台 *", meetingUrl: "会议链接", meetingId: "会议 ID", password: "会议密码", location: "地点与地址 *", locationPlaceholder: "请输入完整地址", notes: "备注", cancel: "取消", save: "保存并推进状态", saving: "保存中..." },
} as const;
