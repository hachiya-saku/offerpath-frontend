...
import { getInterviewsAPI } from "@/api/interviews";
import type { InterviewWithJob } from "@/types/interviews";
...
export function Interviews() {
  ...
  return (
    <div className="grid gap-7">
      ...
      {isLoading ? (
        <p className="rounded-md border border-[#211e25] bg-[#151318] p-6 text-xs text-[#8f8998]">
          {text.loading}
        </p>
      ) : loadError ? (
        <p className="rounded-md border border-[#4b2c32] bg-[#28181c] p-6 text-xs text-[#d9878e]" role="alert">
          {text.loadError}
        </p>
      ) : interviews.length === 0 ? (
...
function InterviewSection({ title, items, language, emptyText, muted = false }: { title: string; items: InterviewWithJob[]; language: "ja" | "zh"; emptyText: string; muted?: boolean }) {
  const text = interviewCopy[language];
  return (
...
const interviewCopy = {
  ja: { title: "面接管理", subtitle: "面接日時、参加情報、会場を一か所で確認できます。", upcomingCount: "件の予定", upcoming: "今後の面接", past: "終了した面接", noUpcoming: "予定されている面接はありません。", empty: "面接予定はまだありません", emptyHint: "求人詳細から次の面接を設定すると、ここに表示されます。", viewJobs: "求人一覧を見る", online: "オンライン", offline: "対面", join: "参加する", map: "地図で確認", loading: "面接情報を読み込んでいます...", loadError: "面接情報を取得できませんでした。" },
  zh: { title: "面试管理", subtitle: "集中查看面试时间、参会信息和线下面试地点。", upcomingCount: "场待面试", upcoming: "即将进行", past: "已结束", noUpcoming: "当前没有待进行的面试。", empty: "还没有面试安排", emptyHint: "在岗位详情中安排下一轮面试后，会显示在这里。", viewJobs: "查看岗位", online: "线上", offline: "线下", join: "进入会议", map: "查看地点", loading: "正在读取面试信息...", loadError: "面试信息获取失败。" },
} as const;
