...
const finishedStatuses = new Set<JobStatus>(["OFFER", "REJECTED", "WITHDRAWN"]);
  ...
  const companyRows = useMemo(
    () =>
      companies.map((company) => ({
        ...
        activeJobs: company.jobs.filter((job) => !finishedStatuses.has(job.status))
          .length,
        platforms: [...new Set(company.jobs.map((job) => job.platform))],
        lastUpdatedAt: company.jobs[0]?.updatedAt ?? company.updatedAt,
      })),
    [companies],
  );
    ...
    <div className="grid gap-6">
      ...
      <section className="overflow-hidden rounded-md border border-[#211e25] bg-[#151318]">
        ...
        {isLoading ? (
          ...
          <Message error>{text.loadError}</Message>
        ) : (
          <>
            {filtered.map((company) => (
              <article className="grid min-h-[78px] grid-cols-[minmax(220px,1.4fr)_100px_minmax(160px,.8fr)_120px_120px] items-center gap-4 border-b border-[#211e25] px-5 last:border-b-0 max-[900px]:flex max-[900px]:flex-col max-[900px]:items-stretch max-[900px]:gap-3 max-[900px]:py-4" key={company.id}>
                <div className="flex min-w-0 items-center gap-3">
                  <span className="company-monogram">{company.name.slice(0, 1).toUpperCase()}</span>
                  <div className="min-w-0"><strong className="block truncate text-xs">{company.name}</strong><small className="mt-1 block text-[9px] text-[#6f6977]">{company.activeJobs} {text.activeJobs}</small></div>
                </div>
                <span className="text-xs text-[#c8c2cd]">{company.jobCount} {text.jobs}</span>
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {company.platforms.length === 0 && <span className="text-[10px] text-[#6f6977]">-</span>}
                </div>
                <span className="text-[10px] text-[#77717f]">{formatDate(company.lastUpdatedAt, language)}</span>
            ...
            {filtered.length === 0 && <Message>{text.empty}</Message>}
          </>
        )}
      </section>
    </div>
...
function formatDate(value: string, language: "ja" | "zh") {
  return new Intl.DateTimeFormat(language === "ja" ? "ja-JP" : "zh-CN").format(
    new Date(value),
  );
}

function Message({ children, error = false }: { children: ReactNode; error?: boolean }) {
  return <div className={`grid min-h-40 place-items-center px-5 text-xs ${error ? "text-[#d9878e]" : "text-[#77717f]"}`} role={error ? "alert" : undefined}>{children}</div>;
}
...
const copy = {
  ja: { eyebrow: "COMPANY DIRECTORY", title: "企業一覧", subtitle: "登録済みの求人から企業情報をまとめて確認できます。", search: "企業名を検索", total: "登録企業", active: "選考中の企業", company: "企業", jobs: "求人", activeJobs: "件選考中", platforms: "求人媒体", updated: "最終更新", viewJobs: "詳細を見る", empty: "該当する企業がありません", loading: "企業情報を読み込んでいます...", loadError: "企業情報を取得できませんでした。" },
  zh: { eyebrow: "COMPANY DIRECTORY", title: "公司一览", subtitle: "集中查看已记录岗位所关联的公司。", search: "搜索公司名称", total: "已记录公司", active: "选考中的公司", company: "公司", jobs: "岗位", activeJobs: "个进行中", platforms: "招聘平台", updated: "最近更新", viewJobs: "查看详情", empty: "没有符合条件的公司", loading: "正在读取公司信息...", loadError: "公司信息获取失败。" },
} as const;
